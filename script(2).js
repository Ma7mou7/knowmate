...
function sub(){
  ...
  // Reliable fallback: poll the room directly. This makes Create/Join work even if Realtime is delayed or unavailable.
  ...
  roomPoll=setInterval(async()=>{
    ...
    const changed=data.player2_id!==room.player2_id||data.status!==room.status||data.player2_name!==room.player2_name;
    room=data;
    if(changed) ui();
    syncPhase();
  },800);
...
async function checkGuesses(){let {data}=await db.from("guesses").select("*").eq("room_id",room.id);if(data&&data.length>=20)results(data)}
